...
def get_movie_detail():
    response = fetch_detail(endpoint.get_movie_detail(api_key = request.args.get('api_key'),mid = request.args.get('mid')))
    return jsonify({
        "result" : response
    })
...
@cache.cached(timeout=300, key_prefix=lambda: request.full_path) 
def get_movie_media():
    response = fetch_video(endpoint.get_media(api_key = request.args.get('api_key'),mid = request.args.get('mid'), media_type = 'video'))
    return jsonify({
        "result" : response
    })
